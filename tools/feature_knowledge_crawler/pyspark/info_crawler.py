...
def python_crawler(origin_category, title, html, dic_filename):
    ...
    if ((origin_category == 'Plot') or (origin_category == 'Style')):
        ...
        WebDriverWait(driver, timeout)
        soup = BeautifulSoup(driver.page_source, "html.parser")
        id = origin_category.lower().replace("_", "-").replace("/", "-").replace(" ", "-")
        soup_section = soup.find("section", id=id)
        ...
        for row in rows:
            ...
            if (origin_category == "Data Types"):
                section_id = func_name.lower().replace('.', '-').replace('_', '-')
                detail = extract_function_detail(full_url, section_id, href)
            else:
                ...
                id = re.sub(r"-{2,}", "-", id)
                section_id = id.strip("-")
                detail = extract_function_detail(full_url, None, href)
            ...
            if (examples == None):
                examples = detail_description["examples"]
...
def extract_function_detail(url, feature_name, href):
    ...
    if not section:
        ...
        driver.quit()
        return {"Examples": None, "Parameters": [], "Returns": []}

    examples, parameters, returns = [], [], []

    examples_header = section.find("p", class_="rubric", string=lambda s: s and s.strip().lower() == "examples")
    if examples_header:
        for sib in examples_header.find_all_next():
            if sib.name == "section":
                break
            if sib.name in ("p", "h2", "h3") and "rubric" in (sib.get("class") or []):
                if sib is not examples_header:
                    break

            if sib.name == "div" and {"doctest", "highlight", "highlight-default"} & set(sib.get("class") or []):
                pre = sib.find("pre")
                code_text = (pre.get_text() if pre else sib.get_text()).strip()
                if code_text:
    ...
    with open(htmls_filename, "r", encoding="utf-8") as rf:
        ...
        for category_key, value in html_contents.items():
            for statement_key, statement_value in value.items():
                if statement_key in ["Core Classes", "Spark Session", "Configuration", "Input/Output", "DataFrame", "Column", "Data Types", "Row"]:
                    print(statement_key + "finished")
                    continue
                print(statement_key+":"+str(statement_value))
                if feature_type == "spark_sql":
                    python_crawler(statement_key, statement_key, statement_value, dic_filename)
                elif feature_type == "pandas_api":
                    python_crawler(statement_key, statement_key, statement_value, dic_filename)
                print('----------------------')
