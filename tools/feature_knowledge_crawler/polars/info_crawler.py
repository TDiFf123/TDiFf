...
def python_crawler(origin_category, title, html, dic_filename):
    ...
    if ((origin_category == 'Plot') or (origin_category == 'Style')):
        ...
        WebDriverWait(driver, timeout)
        soup = BeautifulSoup(driver.page_source, "html.parser")
        id = origin_category.lower().replace("_", "-").replace("/", "-")
        if (html == "https://docs.pola.rs/api/python/stable/reference/expressions/columns.html"):
            id = "columns-names"
        soup_section = soup.find("section", id=id)
...
def extract_description_and_examples(url, section_id):
    ...
    if not section:
        print(f"Section '{section_id}' not found in {url}")
        return None
...
def extract_function_detail(url, feature_name, href):
    ...
    if examples_header:
        code_blocks = examples_header.find_all_next("div", class_="highlight")
        for code_block in code_blocks:
            if code_block:
                example_code = code_block.get_text().strip()
                example_code = example_code.replace("Copy to clipboard", "").strip()
    ...
    with open(htmls_filename, "r", encoding="utf-8") as rf:
        ...
        for category_key, value in html_contents.items():
            for statement_key, statement_value in value.items():
                ...
                elif feature_type == "set_operations":
                    function_crawler(statement_key, statement_key, statement_value, dic_filename)
                elif feature_type == "table_operations":
                    function_crawler(statement_key, statement_key, statement_value, dic_filename)
                elif feature_type == "sql_clauses":
                    function_crawler(statement_key, statement_key, statement_value, dic_filename)
                elif feature_type == "dataframe":
                    python_crawler(statement_key, statement_key, statement_value, dic_filename)
                elif feature_type == "expressions":
                    python_crawler(statement_key, statement_key, statement_value, dic_filename)
                elif feature_type == "data-types":
                    python_crawler(statement_key, statement_key, statement_value, dic_filename)
                print('----------------------')
