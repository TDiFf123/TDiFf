...
def exec_sql_statement(original_sql, con):
    try:
        sql_result = con.sql(original_sql).fetchall()
...
def exec_python_statement(spark, transfer_code):
    try:
...
def _row_to_py(obj):
    """Recursively convert Spark Rows to Python serializable structures."""
...
def rows_to_list(rows: Optional[List[Row]]) -> Optional[List[Any]]:
    """Convert a list of Spark Rows into a list for JSON serialization."""
    if rows is None:
        return None
    out = []
    for r in rows:
...
def llm_generate_transferred_sql(sql_statement, origin_db, conversation, iteration_num, tables, con, spark, model, method, prompt_path, source_data_path, dfg_host_port, csv_context="", feature=False, DFG=False, Explanation=True):
...
With this expertise, translate the following Duckdb SQL statement into equivalent executable Python code using the Spark DataFrame API while preserving the original semantics.
...
4. For any referenced SQL table, obtain it via spark.table("<table_name>"); import PySpark SQL functions as F (from pyspark.sql import functions as F).
...
"""
    ...
    while conversation_cnt < iteration_num:
        ...
        python_result, python_error_messages = exec_python_statement(spark, python_statement)
        if Explanation:
        ...
        iteration_history.append({
            ...
            "ExecutionResult": rows_to_list(python_result)
        })
    ...
    return {
        ...
        "FinalPythonResult": rows_to_list(final_python_result)
    }
...
def main(db, sql, con, spark, output_path, prompt_path, source_data_path, iteration_num, model, reasoning, temperature, method, base_url, csv_dir, api_key, tables, csv_context, dfg_host_port, feature=False, DFG=False, Explanation=True):
    chat_kwargs = dict(
        model=model,
        base_url=base_url,
        api_key=api_key,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--include_csv_context", type=str_to_bool, default=True)
    parser.add_argument("--feature", type=str_to_bool, default=True)
    parser.add_argument("--DFG", type=str_to_bool, default=True)
    ...
    init_spark_tables(args.csv_dir, spark)
    ...
    for i, sql in enumerate(sql_statements):
        ...
        try:
            main(args.db, sql, con, spark, result_path, prompt_path, source_data_path, args.iteration_num, model, args.reasoning, args.temperature, args.method, base_url, args.csv_dir, args.api_key, tables, csv_context, args.dfg_host_port, feature=args.feature, DFG=args.DFG, Explanation=args.Explanation)
        except Exception as e:
            print(e)
            continue
