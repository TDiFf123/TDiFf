...
def init_polars_tables(csv_dir):
    tables = {}
    for fname in os.listdir(csv_dir):
        if fname.endswith(".csv"):
            table_name = os.path.splitext(fname)[0]
            path = os.path.join(csv_dir, fname)
            tables[table_name] = pl.read_csv(path)
            # print(f"Loaded {table_name} from {path}")
    return tables
...
def exec_python_statement(transfer_code, tables):
    try:
        local_vars = {name: df.clone() for name, df in tables.items()}
        local_vars["pl"] = pl
...
def llm_generate_transferred_sql(sql_statement, origin_db, conversation, iteration_num, tables, con, model, method, prompt_path, source_data_path, dfg_host_port, csv_context="", feature=False, DFG=False, Explanation=True):
    ...
    if DFG:
        ...
        No = No + 1
        require = require + f"{No}. When the SQL query's `SELECT` clause contains only constants, functions, calculation results, etc. without referencing any columns, it MUST use `with_columns(pl.lit(...).alias(\"col_name\"))` to broadcast the constant column to each row of the table, ensuring that the row count matches that of the original tables.\n"
    else:
        dfg = ""
        sql_query_plan = None
...
With this expertise, translate the following Duckdb SQL statement into equivalent executable Python code using the Polars DataFrame API while preserving the original semantics.
...
def default_serializer(obj):
    if isinstance(obj, (datetime.datetime, datetime.date)):
        ...
        return obj.to_dicts() if isinstance(obj, pl.DataFrame) else obj.to_dict(orient="records")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dfg_host_port", default='http://localhost:8083/dot-to-ascii.php')
    ...
    base_result_dir = os.path.join(os.getcwd(), "transfer_result1")
    ...
    pandas_tables, csv_context = init_duckdb_tables(args.csv_dir, con, args.include_csv_context)
    polar_tables = init_polars_tables(args.csv_dir)
