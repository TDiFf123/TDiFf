...
def compare_values_only(sql_res, py_res):
    """
    Compare only values of SQLExecutionResult and FinalPythonResult.
    Ignore column names.
    Ignore row ordering (multiset comparison).
    ...
    # Convert each row to stable comparable string and sort
    sql_norm_sorted = sorted([json.dumps(row, sort_keys=True, ensure_ascii=False) for row in sql_norm])
    py_norm_sorted  = sorted([json.dumps(row, sort_keys=True, ensure_ascii=False) for row in py_norm])
